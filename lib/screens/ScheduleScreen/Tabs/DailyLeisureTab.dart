...
class DailyLeisureTab extends StatefulWidget {
  DailyLeisureTab();

  @override
  _DailyLeisureTabState createState() => _DailyLeisureTabState();
}

class _DailyLeisureTabState extends State<DailyLeisureTab> {
  ...
  List<ScheduleTask> todayLeisureTaskList = List();
  @override
  void initState() {
    super.initState();
    baseAuthentication.getCurrentUser().then((user) {
      ...
      dailyTaskSubscription = scheduleService
        ...
        final List<ScheduleTask> tasks = snapshot.documents
            ...
            .toList();
        setState(() {
          this.todayLeisureTaskList = tasks;
        });
      });
...
Card makeCard(ScheduleTask leisureTask) => Card(
          elevation: 8.0,
          margin: new EdgeInsets.symmetric(horizontal: 10.0, vertical: 6.0),
          child: Slidable(child: Container(
            decoration: BoxDecoration(color: Colors.deepPurpleAccent),
            child: buildTilesList(leisureTask),
          ),
          ...
          secondaryActions: <Widget>[
            ...
            icon: Icons.create,
            onTap: ()=> addTaskProgress(leisureTask),),
          ],)
         
        );

    final dailyLeisureTabBody = Container(
      child: ListView.builder(
        scrollDirection: Axis.vertical,
        shrinkWrap: true,
        itemCount: todayLeisureTaskList.length,
        itemBuilder: (BuildContext context, int index) {
          return makeCard(todayLeisureTaskList[index]);
        },
      ),
    );
    return Scaffold(
      body: dailyLeisureTabBody,
