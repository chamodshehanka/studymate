...
class _TuesdayTabState extends State<TuesdayTab> {
  ...
  List<ScheduleTask> tuesdayTaskList = List();
  @override
  void initState() {
        super.initState();
...
socialActivitySubscription?.cancel();
 ...
 Card makeCard(ScheduleTask tuesdayTask) => Card(
          elevation: 8.0,
          margin: new EdgeInsets.symmetric(horizontal: 10.0, vertical: 6.0),
          child: Slidable(child: Container(
            decoration: BoxDecoration(color: Colors.deepPurpleAccent),
            child: buildTilesList(tuesdayTask),
          ),
          ...
          secondaryActions: <Widget>[
            ...
            icon: Icons.delete,
            onTap: ()=> deleteTask(tuesdayTask),),
            IconSlideAction(caption: 'Update',
            color: Colors.yellowAccent,
            icon: Icons.update,
            onTap: ()=> updateTask(tuesdayTask,socialActivities,leisureActivities),)
          ],)
         
        );

    final tuesdayTabBody = Container(
      child: ListView.builder(
        scrollDirection: Axis.vertical,
        shrinkWrap: true,
        itemCount: tuesdayTaskList.length,
        itemBuilder: (BuildContext context, int index) {
          return makeCard(tuesdayTaskList[index]);
        },
      ),
    );
    return Scaffold(
 ...
 void deleteTask(ScheduleTask task) {
      ...
      Future<dynamic> isDeleted =
          scheduleService.deleteTask(studentId, "tuesday", task.id.toString());
            ...
            Padding(
              ...
              child: StudymateRaisedButton("Add To Schedule", () {
                if (_formKey.currentState.validate()) {
                  ...
                  ScheduleService()
                      .addToSchedule(widget.studentId, scheduleTask, "tuesday");
                      Navigator.pop(context);
                }
              }, Colors.deepPurple),
            )
                                ...
                                child: StudymateRaisedButton("Update Task", () {
                                  if (_formKey.currentState.validate()) {
                                    ...
                                    ScheduleTask scheduleTask = new ScheduleTask(
                                     ...
                                     ScheduleService()
                                        .updateTask(widget.studentId, scheduleTask, "tuesday");
                                        Navigator.pop(context);
                                  }
                                }, Colors.deepPurple),
                              )
