...
class _FridayTabState extends State<FridayTab> {
  ...
  List<ScheduleTask> fridayTaskList = List();
  @override
  void initState() {
    super.initState();
    baseAuthentication.getCurrentUser().then((user) {
      ...
      dailyTaskSubscription = scheduleService
          .getDailyTaskList(studentId, 'friday')
...
Card makeCard(ScheduleTask fridayTask) => Card(
          elevation: 8.0,
          margin: new EdgeInsets.symmetric(horizontal: 10.0, vertical: 6.0),
          child: Slidable(child: Container(
            decoration: BoxDecoration(color: Colors.deepPurpleAccent),
            child: buildTilesList(fridayTask),
          ),
          ...
          secondaryActions: <Widget>[
            ...
            icon: Icons.delete,
            onTap: ()=> deleteTask(fridayTask),),
            IconSlideAction(caption: 'Update',
            color: Colors.yellowAccent,
            icon: Icons.update,
            onTap: ()=> updateTask(fridayTask,socialActivities,leisureActivities),)
          ],)
         
        );

    final fridayTabBody = Container(
      child: ListView.builder(
        scrollDirection: Axis.vertical,
        shrinkWrap: true,
        itemCount: fridayTaskList.length,
        itemBuilder: (BuildContext context, int index) {
          return makeCard(fridayTaskList[index]);
        },
      ),
    );
    return Scaffold(
